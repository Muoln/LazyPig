local CheckBoxTables = {
	...
	["Naxx Roll Automation"] = {
		...
		[1] = { "LazyPigCheckbox13", "Need" },
		[2] = { "LazyPigCheckbox14", "Greed" },
		[3] = { "LazyPigCheckbox15", "Pass" }
	},
	...
	["AQ Mount Automation"] = {
		[0] = "LazyPigCheckboxGroupAQMount",
		[1] = { "LazyPigCheckbox19", "Pass" },
	},
	...
	["Smart Salvation Remover"] = {
		...
		[2] = { "LazyPigCheckbox61", "Warrior Shield/Bear Form/Righteous Fury/Rockbiter" },
	},
...
function LazyPig_CreateOptionsFrame()
	...
	fs_title:SetText("LazyPig Options - Klutch Edition")
	...
	frame.cbgroup_mcroll = CheckBoxGroup(frame, 20, -105, str, CheckBoxTables[str])
	
	local str = "Zul'Gurub Roll Automation"
	frame.cbgroup_zgroll = CheckBoxGroup(frame, 20, -165, str, CheckBoxTables[str])
	
	local str = "AQ Idols + Scarabs Automation"
	frame.cbgroup_aq = CheckBoxGroup(frame, 20, -225, str, CheckBoxTables[str])
	
	local str = "AQ Mount Automation"
	frame.cbgroup_aqmount = CheckBoxGroup(frame, 20, -285, str, CheckBoxTables[str])

	local str = "Naxx Roll Automation"
	frame.cbgroup_naxxroll = CheckBoxGroup(frame, 20, -315, str, CheckBoxTables[str])
	
	local str = "Battlegrounds Automation"
	frame.cbgroup_bgautomation = CheckBoxGroup(frame, 20, -375, str, CheckBoxTables[str])
	
	local str = "World Chat Mute"
	frame.cbgroup_worldchatmute = CheckBoxGroup(frame, 20, -480, str, CheckBoxTables[str])
	...
	frame.cbgroup_nameplates = CheckBoxGroup(frame, 250, -415, str, CheckBoxTables[str])
