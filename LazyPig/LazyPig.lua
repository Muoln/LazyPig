...
local LazyPigMenuStrings = {
		...
		[60]= "Always",
		[61]= "Warrior Shield/Bear Form/Righteous Fury/Rockbiter",
...
function LazyPig_OnEvent(event)
	...
	elseif(event == "CHAT_MSG_LOOT") then
		if (string.find(arg1 ,"You won") or string.find(arg1 ,"You receive")) and (string.find(arg1 ,"cffa335e") or string.find(arg1, "cff0070d") or string.find(arg1, "cffff840")) and not string.find(arg1 ,"Bijou") 
			and not string.find(arg1 ,"Idol") and not string.find(arg1 ,"Shard") and not string.find(arg1, "Wartorn") and not string.find(arg1, "Thawing") and not string.find(arg1, "Blue Qiraji Resonating Crystal") 
			and not string.find(arg1, "Green Qiraji Resonating Crystal") and not string.find(arg1, "Yellow Qiraji Resonating Crystal") and not string.find(arg1, "Scarab") and not string.find(arg1, "Fiery Core") 
			and not string.find(arg1, "Lava Core") then
			save_time = GetTime()
		end
...
function LazyPig_AutoRoll(id)
	...
	LazyPig_NaxxRoll(id)
	LazyPig_AQRoll(id)
	LazyPig_AQMountRoll(id)
end

-- Function for auto rolling on MC cores

function LazyPig_MCRoll(id)
	RollReturn = function()
		local txt = ""
		if LPCONFIG.MC == 1 then
			txt = "NEED"
		elseif LPCONFIG.MC == 2 then
			txt = "GREED"
		elseif LPCONFIG.MC == 0 then
			txt = "PASS"
		end
		return txt
	end
	if LPCONFIG.MC then	
		...
		if string.find(name, "Fiery Core") or string.find(name, "Lava Core") then
...
-- Function for auto rolling on ZG bijous and coins
...
-- Function for auto rolling on Naxx t3 scrap items and runes
function LazyPig_NaxxRoll(id)
	RollReturn = function()
		local txt = ""
		if LPCONFIG.NAXX == 1 then
			txt = "NEED"
		elseif LPCONFIG.NAXX == 2 then
			txt = "GREED"
		elseif LPCONFIG.NAXX == 0 then
			txt = "PASS"
		end
		return txt
	end
...
-- Function for auto rolling on AQ scarabs and coins
...
-- Function for passing shit AQ mounts (Not Red)
function LazyPig_AQMountRoll(id)
	RollReturn = function()
		local txt = ""
		if LPCONFIG.AQ_MOUNT == 0 then
			txt = "PASS"
		end
		return txt
	end
	if LPCONFIG.AQ_MOUNT then	
		local _, name, _, quality = GetLootRollItemInfo(id);
		if string.find(name, "Blue Qiraji Resonating") or string.find(name, "Green Qiraji Resonating")
			or string.find(name, "Yellow Qiraji Resonating") then
			RollOnLoot(id, LPCONFIG.AQ_MOUNT);
			local _, _, _, hex = GetItemQualityColor(quality)
			DEFAULT_CHAT_FRAME:AddMessage("LazyPig: Auto "..hex..RollReturn().." "..GetLootRollItemLink(id))
			return
		end	
...
function LazyPig_GetOption(num)
	...
	or num == 03 and LPCONFIG.MC == 1
	or num == 04 and LPCONFIG.MC == 2 
	or num == 05 and LPCONFIG.MC == 0 	
	or num == 10 and LPCONFIG.ZG == 1		
	or num == 11 and LPCONFIG.ZG == 2
	or num == 12 and LPCONFIG.ZG == 0
	or num == 13 and LPCONFIG.NAXX == 1
	or num == 14 and LPCONFIG.NAXX == 2
	or num == 15 and LPCONFIG.NAXX == 0
	or num == 16 and LPCONFIG.AQ == 1
	or num == 17 and LPCONFIG.AQ == 2
	or num == 18 and LPCONFIG.AQ == 0
	or num == 19 and LPCONFIG.AQ_MOUNT == 0
...
function LazyPig_SetOption(num)
	...
	elseif num == 19 then
		LPCONFIG.AQ_MOUNT = 0
		if not checked then LPCONFIG.AQ_MOUNT = nil end
	elseif num == 20 then															
...
function LazyPig_ChatFrame_OnEvent(event)
	if event == "CHAT_MSG_LOOT" or event == "CHAT_MSG_MONEY" then
		local core = string.find(arg1, "Lava Core") or string.find(arg1, "Fiery Core")
		local bijou = string.find(arg1 ,"Bijou")
		local coin = string.find(arg1 ,"Coin")
		local scrap = string.find(arg1, "Wartorn")
		local thawing = string.find(arg1, "Thawing")
		local idol = string.find(arg1, "Idol")
		local scarab = string.find(arg1, "Scarab")
		local mounts = string.find(arg1, "Blue Qiraji Resonating") or string.find(arg1, "Green Qiraji Resonating") or string.find(arg1, "Yellow Qiraji Resonating")
		...
		local checkMC = LPCONFIG.MC and core
		local checkZG = LPCONFIG.ZG and (bijou or coin)
		local checkNaxx = LPCONFIG.NAXX and (scrap or thawing)
		local checkAQ = LPCONFIG.AQ and (idol or scarab)
		local checkAQMount = LPCONFIG.AQ_MOUNT and mounts
		local check4 = check1 and not checkMC and not checkZG and not green_roll and not checkNaxx and not checkAQ and not checkAQMount or check2 
